...
class TestStripeBasicFunctions:
    ...
    def test_4_account_details_verification(self):
        ...
        try:
            account = stripe.Account.retrieve(chad_account['id'])
...
class TestStripeTransferFunctions:
    ...
    def test_9_simulate_transfer_without_execution(self):
        ...
        try:
            chad_stripe = stripe.Account.retrieve(chad['id'])
            juan_stripe = stripe.Account.retrieve(juan['id'])
            
            # Verificar que pueden recibir transferencias
            chad_can_send = chad_stripe.charges_enabled
            juan_can_receive = juan_stripe.capabilities.transfers == 'active'
